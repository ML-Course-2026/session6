...
This laboratory provides step-by-step instructions for fine-tuning the `Qwen/Qwen2.5-1.5B-Instruct` model using Low-Rank Adaptation (LoRA).
...
Before we train the model, let's ask it our target question. Because the base model has never seen your `MediCore.json` dataset, it will likely give a generic or hallucinated answer about MediCore Hospital. This establishes a baseline to prove our fine-tuning works.
...
*  `-nc` (no-clobber) ensures that if the student *did* manually upload their own version of `MediCore.json`, the command won't overwrite it. 
...
<br>
The generation prompt adds the "assistant's turn to speak" header at the very end of the text. During training, we are showing the model a completed transcript of both the user's question and the assistant's answer, so we don't need a prompt at the end asking the model to generate anything new. 
</details>
...
*   `lora_alpha=32`: The scaling factor. This dictates how strongly the new LoRA weights influence the original base model. A standard rule of thumb is to set `alpha` to double the `r` value.
...
*   `get_peft_model(model, lora_config)`: Wraps the massive, frozen base model and the tiny, trainable LoRA matrices together into a single manageable object.
...
Once training is complete, the LoRA adapters must be loaded alongside the base model. This cell simulates what you would do if you restarted your Colab notebook and wanted to load your saved work.
...
#### Code Explanation:
*   `PeftConfig.from_pretrained(path)`: Reads the configuration file saved in your `./my_qwen` folder to determine exactly which base model (e.g., `Qwen2.5-1.5B`) these adapters were trained on.
*   `AutoModelForCausalLM.from_pretrained(...)`: Reloads the massive, original 1.5 billion parameters of the base model into the GPU memory.
*   `PeftModel.from_pretrained(base_model, path)`: This is the magic of LoRA. It takes the heavy base model and dynamically "snaps on" your tiny, fine-tuned adapter matrices.
...
<details>
<summary><b>Q&A: Why do we have to load the base model again? Why didn't `trainer.save_model` save everything?</b></summary>
<br>
Because we used LoRA, <code>trainer.save_model</code> only saved the tiny adapter weights (the "diff" or changes learned during training), which are usually just a few megabytes. It does not duplicate the 3 GB base model to save hard drive space. Therefore, to run the model, you must load the foundation (the base model) and then attach the roof (your LoRA adapter).
</details>
...
*   `max_new_tokens=100`: Puts a hard limit on the response length so the model doesn't generate endless text if it gets confused.
...
### Step 8: (Optional) Merge Adapters for Deployment

Loading the base model and the adapter separately is fine for testing. However, if you want to deploy this model to an app or UI (like Gradio), it is standard practice to fuse the LoRA weights permanently into the base model.
...
#### Code Explanation:
*  `model.merge_and_unload()`: Mathematically adds the LoRA adapter weights directly into the corresponding base model weight matrices. The adapter is then discarded. The result is a single standalone model with no PEFT dependency — simpler to deploy.
*  `merged_model.save_pretrained(...)`: Saves the full merged model (base + adapter fused). Unlike `trainer.save_model`, this saves the **entire model** (~3 GB), not just the adapter.
...
*   **For Markdown:** Gradio's standard `gr.Chatbot()` or `gr.Markdown()` components parse and render Markdown natively. If your model outputs `**Hello**`, Gradio will automatically display it as **Hello**. You don't need to write any extra code.
...
## Concept: Fine-Tuning vs. Retrieval-Augmented Generation (RAG)
